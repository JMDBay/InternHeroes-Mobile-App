import 'dart:io';
import 'package:InternHeroes/features/user_auth/presentation/pages/onboardingscreen.dart';
...
class _AdditionalInformationPageState
  ...
  Future<String> _uploadDefaultImageToFirebaseStorage() async {
    try {
      // Path to the default image asset
      String defaultImagePath = 'assets/images/superhero3.jpg';

      // Read the default image as bytes
      ByteData imageData = await rootBundle.load(defaultImagePath);
      List<int> bytes = imageData.buffer.asUint8List();

      // Convert List<int> to Uint8List
      Uint8List data = Uint8List.fromList(bytes);

      // Upload the default image bytes to Firebase Storage
      String fileName = Path.basename(defaultImagePath);
      firebase_storage.Reference firebaseStorageRef =
      firebase_storage.FirebaseStorage.instance.ref().child(fileName);
      firebase_storage.UploadTask uploadTask =
      firebaseStorageRef.putData(data);
      await uploadTask.whenComplete(() {});
      String downloadURL = await firebaseStorageRef.getDownloadURL();
      return downloadURL;
    } catch (e) {
      print('Error uploading default image to Firebase Storage: $e');
      return ''; // Return an empty string if there's an error
    }
  }
    ...
    if (user != null) {
      try {
        ...
        if (_selectedImage != null) {
          ...
          profileImageUrl = await _uploadDefaultImageToFirebaseStorage();
        }
                ...
                child: Container(
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      ...
                      width: 1.0, // Thinner border width
                    ),
                  ),
                  child: CircleAvatar(
                    radius: 60,
                    backgroundImage: _selectedImage != null
                        ...
                        : AssetImage('assets/images/superhero3.jpg') as ImageProvider,
                  ),
                ),
              ),
            ),
